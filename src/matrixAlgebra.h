...
#if defined(USE_EIGEN) && USE_EIGEN
...
inline double squaredNorm(const vector &v) { return v.squaredNorm(); }
} // namespace FittingAlgorithms
...
inline matrix transpose(const matrix &M) { return ublas::trans(M); }
...
inline double squaredNorm(const vector &v) { return ublas::inner_prod(v, v); }
} // namespace FittingAlgorithms
#endif
