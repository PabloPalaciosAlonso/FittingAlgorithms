...
int main(int argc, char *argv[]) {
  ...
  // Suposiciones iniciales (una por cada temperatura)
  StringDoubleMap seed = {{"a", 1.0}, {"b", 1.0}, {"c", 6.0}};
  std::vector<StringDoubleMap> initialGuesses(ptParams.temperatures.size(),
                                              seed);

  auto result = ParallelTempering::fit<double, double, StringDoubleMap>(xdata, ydata, model,
