...
struct Parameters {
  std::vector<double>
      temperatures; ///< Vector of the Monte Carlo (MC) temperatures.
  std::vector<double>
      jumpSize;       ///< Relative step size for each parameter being fitted.
  int numStepsSwap;   ///< Number of Metropolis-Hastings steps between attempted
                      ///< chain swaps.
  int numStepsFinish; ///< Number of steps after which execution terminates if
                      ///< no new global minimum has been found.
...
#if defined(FA_OPENMP_ENABLED)
  int numThreads = omp_get_max_threads(); ///< Number of threads to use for
                                          ///< parallel execution.
#else
  int numThreads = 1; ///< Single-thread fallback when OpenMP is disabled.
#endif
};

/**
 * @brief Fits the parameters of a model to data using the Parallel Tempering
 * algorithm.
 *
 ...
 * @param initialGuesses Vector of initial parameter maps for each chain (must
 * match params.temperatures.size()).
...
template <class T>
StringDoubleMap
fit(std::vector<T> &xdata_in, std::vector<double> &ydata_in,
    ModelFunction<T> model, std::vector<StringDoubleMap> &initialGuesses,
    Parameters params = Parameters(), CostFunction costFunction = squaredError,
...
#pragma omp parallel for num_threads(params.numThreads)
    ...
    for (int tempIdx = 0; tempIdx < nTemperatures; ++tempIdx) {
      errors[tempIdx] = forwardTimeMC(
          xdata_in, ydata_in, model, allFittingParameters[tempIdx],
          params.temperatures[tempIdx], params.jumpSize[tempIdx], costFunction,
          extraParameters);
    ...
    if (step % params.printSteps == 0 and step > 0) {
      ...
      printStepProgress(step, params.maxIterations, start, elapsed.count(),
                        optimalErrors, optimalFittingParameters);
      start = now;
    }
    ...
    if (step % params.numStepsSwap == 0 and step > 0) {
      ...
      if (stepsSameError >= params.numStepsFinish ||
          minError < params.tolerance) {
        break;
      }
    }
  }
